...
const ChatUI = () => {
    const [list, setlist] = useState([
        {
            type: 'receiver',
            sender: "https://images.unsplash.com/photo-1549078642-b2ba4bda0cdb?ixlib=rb-1.2.1&amp;ixid=eyJhcHBfaWQiOjEyMDd9&amp;auto=format&amp;fit=facearea&amp;facepad=3&amp;w=144&amp;h=144",
            message: [
            ...
            <div id="messages" class="flex flex-col space-y-4 p-3 overflow-y-auto scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2 scrolling-touch">
                {list.map((el, index) => (
                    <div class="chat-message" key={index}>
                        <div class={`flex items-end ${el.type === 'sender'? 'justify-end': ''}`}>
                            <div class={`flex flex-col space-y-2 text-xs max-w-xs mx-2 ${el.type === 'sender' ? 'order-1 items-end' : 'order-2 items-start'}`}>
                                {el.message.map((msg, _index) => (
                                    <div key={_index}>
                                        <span class={`px-4 py-2 rounded-lg inline-block rounded-br-none ${el.type === 'sender'? 'bg-blue-600 text-white' : 'bg-gray-300 text-gray-600'}`}>
                                            {msg}
                                        </span>
                                    </div>
                                ))}
